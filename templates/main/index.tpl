  ...
  <table width="100%" border="0" align="center" id="clients" class="table_user" cellpadding="0" cellspacing="1">
	...
	{if $main_query}
      {foreach from=$main_query item=item}
         <tr onmousemove="show_edit({$item.id})" class="{if $item.cur_st=='1'}color_st1{/if}{if $item.cur_st=='5'}color_st5{/if}{if $item.cur_st=='2'}color_st2{/if}{if $item.cur_st=='-1'}color_st3{/if}{if $item.cur_st=='-2'}color_st4{/if}">
      		<td style="width:20px; vertical-align:middle;"><b>{$item.id}</b></td>
			...
			<select name="cur_st" id="cur_st" onchange="get_st(this,{$item.id},{if $display==''}0{else}{$display}{/if})">
				...
				Тип ремонта - <b>{$item.repair_type}</b>
      		</td>
      		<td style="width:15%; max-width:15%;"><a href="?page=show_agent&id={$item.id_a}"><b>{$item.fio}</b></a></td>
      		<td width="10%"><span id="client_phone">{$item.tel}</span></td>
            ...
            <td style="width:20%; max-width:20%;">
      		...
      		<td width="10%">
			<input type="text" class="stext_hard" name="summa_hard" value="{$item.summa_hard}" placeholder="за железки" onchange="pay_hard(this,{$item.id})">
			<input type="text" class="stext_soft" name="summa" value="{$item.summa}" placeholder="за работу" onchange="pay(this,{$item.id})">
      		</td>
      	</tr>
      	{/foreach}
      {else}
